...
public class CarRentDataAdapter extends RecyclerView.Adapter {
    private final LayoutInflater inflater;
    private final List<Rent> values;
    private Context context;
    FragmentManager fragmentManager;
    View button;


    public CarRentDataAdapter(Context context ) {
        this.inflater = LayoutInflater.from(context);
        this.context=context;
        values=new ArrayList<>();
    }
    public void init(List<Rent> rents, FragmentManager fragmentManager){
    ...
    public RecyclerView.ViewHolder onCreateViewHolder(@NonNull ViewGroup viewGroup, int i) {
        View view = inflater.inflate(R.layout.item_rent, viewGroup, false);
        return new CarRentDataAdapter.ViewHolder(view);
    }
    ...
    public void onBindViewHolder(@NonNull RecyclerView.ViewHolder viewHolder, int i) {
        final Rent r = (Rent) values.get(i);
...
//                dialogFragment.show(fragmentManager, "d");
        ...
        for (int i = 0; i < values.size(); i++) {
            if (values.get(i).isChecked()) {
                ...
                try {
                    ...
                    object.put("cost", ((Rent) values.get(i)).getCost());
                    selected.put(object);
                } catch (Exception e) {
                    e.printStackTrace();
                }
    ...
    public class ViewHolder extends RecyclerView.ViewHolder {
        ...
        final TextView name, start, end, cost;
        RelativeLayout gen;
        CheckBox check;
